...
// patch-v7 신규 — 공통 사이드바 + Record/Analysis 페이지 + Focus 실시간 차트
import { LeftSidebar } from './components/LeftSidebar.jsx';
import { RecordPage } from './components/RecordPage.jsx';
import { AnalysisPage } from './components/AnalysisPage.jsx';
import { FocusRealtimeChart } from './components/FocusRealtimeChart.jsx';
  ...
  return (
    <div className="page">
      <div className="focus-wrap">
        ...
        {/* 실시간 페이스 차트 (patch-v7) — 1초마다 갱신, 예상선 vs 실제선 */}
        <div style={{ marginTop: 24 }}>
          <FocusRealtimeChart plan={plan} currentItem={current} />
        </div>
...
/* ═══════════════════════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════════════════════ */
/* ═══════════════════════════════════════════════════════════════
   PageWithSidebar — Record / Analysis 공통 레이아웃 (patch-v7)
   좌측 LeftSidebar + 우측 페이지 콘텐츠. 메인과 동일한 그리드.
   ═══════════════════════════════════════════════════════════════ */
function PageWithSidebar({ records, onNavigate, children }) {
  return (
    <div style={{
      maxWidth: 1280,
      margin: '0 auto',
      ...
      <div className="page-grid" style={{
        ...
        <LeftSidebar records={records} onNavigate={onNavigate} />
...
function AppMain() {
  ...
  const pg = () => {
    ...
    switch (path) {
      ...
      case "/record": return (
        <PageWithSidebar records={records} onNavigate={nav}>
          <RecordPage records={records} savedIds={savedIds} onToggleSave={toggleSave} onNavigate={nav} />
        </PageWithSidebar>
      );
      case "/analysis": return (
        <PageWithSidebar records={records} onNavigate={nav}>
          <AnalysisPage records={records} />
        </PageWithSidebar>
      );
