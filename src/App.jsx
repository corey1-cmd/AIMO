...
/* ═══════════════════════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════════════════════ */
function MainPage({ onNavigate, records, activePlan, onStartPlan, onClosePlan }) {
    ...
    <div style={{
      ...
      <div className="main-grid" style={{
        ...
        <aside style={sidebarBox()}>
          ...
          <SidebarDivider />
          <SidebarSection padding="18px 18px">
        ...
        <div style={rightBox()}>
          ...
          <BoxContent>
            <RightPanel
              ...
              onResumeFocus={() => onNavigate('/focus/session')}
...
/* ═══════════════════════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════════════════════ */
function FocusDashboardPage({ onNavigate, records }) {
  const { stats, top3, recent } = useDashboardStats(records);
    ...
    <div style={{
      ...
      <div className="main-grid" style={{
        ...
        <div style={rightBox()}>
          <BoxLightLight />
          <BoxContent>
            <FocusDashboard records={records} />
          </BoxContent>
        </div>
      </div>
...
/* ═══════════════════════════════════════════════════════════════
   FocusSessionPage — /focus/session 라우트
   포커스 세션을 같은 통합 박스 레이아웃 안에 표시.
   사이드바는 records를 실시간으로 반영(MainPage와 동일한 방식).
   ═══════════════════════════════════════════════════════════════ */
function FocusSessionPage({ plan, onUpdatePlan, onCompleteAll, onCancel, onNavigate, records }) {
  const { stats, top3, recent } = useDashboardStats(records);
    ...
    <div style={{
      ...
      <div className="main-grid" style={{
        ...
        <aside style={sidebarBox()}>
          ...
          <SidebarDivider />
          <SidebarSection padding="20px 24px">
            <Top3 items={top3} onSelect={(id) => onNavigate(`/record/${id}`)} />
          </SidebarSection>
          <SidebarDivider />
          <SidebarSection padding="20px 24px">
            <RecentCompleted
...
/* ═══════════════════════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════════════════════ */
function LearningPage({ onNavigate, records }) {
  const { stats, top3, recent } = useDashboardStats(records);
    ...
    <div style={{
      ...
      <div className="main-grid" style={{
        ...
        <aside style={sidebarBox()}>
          ...
          <SidebarDivider />
          <SidebarSection padding="18px 18px"><QuickEntry onNavigate={onNavigate} /></SidebarSection>
          <SidebarDivider />
...
function AppMain() {
  ...
  // 메인 페이지의 우측 패널에서 분석 결과 확정 시 호출.
  // DistillPage 로 가지 않고 그 자리에서 plan 을 등록하고 포커스 세션으로 진입.
  ...
  const pg = () => {
    ...
    switch (path) {
      ...
      case "/focus": return <FocusDashboardPage onNavigate={nav} records={records} />;
      case "/focus/session": return (
        <FocusSessionPage
          plan={activePlan}
      ...
      case "/learning": return <LearningPage onNavigate={nav} records={records} />;
      default: return (
        <div className="page">
          <div className="empty-state">
            <span className="empty-icon">🔍</span>
