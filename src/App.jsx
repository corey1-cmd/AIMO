...
/* ═══════════════════════════════════════════════════════════════
   ...
   Hard Gate (2026-04 patch-v6a-hardgate):
     비로그인 사용자는 LoginPage 만 볼 수 있음.
     loading 상태에서는 빈 화면 (Splash 와 동일 효과, flash 차단).
     authenticated 상태에서만 메인 앱 진입.
   ═══════════════════════════════════════════════════════════════ */
export default function App() {
  const { status } = useAuth();

  // ─── Hard Gate ─────────────────────────────────────────────
  // status === 'loading': 세션 확인 중. 빈 shell 렌더.
  // status === 'unauthenticated': LoginPage 표시.
  // status === 'authenticated': 정상 앱 진입 (아래 본문).
  if (status === 'loading') {
    // Sprout 가이드 일관: bg #f6fbf4 → bgGradientEnd #ebefe8
    // 로그인 페이지 그라디언트와 동일하여 인증 확인 → LoginPage 또는 AppMain 진입 시
    // 배경 색이 점프하지 않음.
    return (
      <div style={{
        minHeight: '100vh',
        ...
        {/* 의도적으로 비워둠 — flash 방지가 목적이지 사용자 정보 표시 아님 */}
      </div>
    );
  }
