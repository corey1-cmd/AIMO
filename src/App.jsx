...
function DistillPage({ onNavigate, onStartPlan }) {
  ...
  const [tasks, setTasks] = useState([{ id: uid(), title: "", date: "", estimatedMin: 30 }]);
  ...
  function addTask() { if (tasks.length >= 7) return; setTasks([...tasks, { id: uid(), title: "", date: "", estimatedMin: 30 }]); }
  ...
  function reset() {
    setPhase(PH.INPUT);
    setTasks([{ id: uid(), title: "", date: "", estimatedMin: 30 }]);
    setBD([]); setAN([]);
  }
            ...
            <div key={task.id} className="task-card" style={{ borderLeft: `3px solid ${PASTELS[i % PASTELS.length].border}` }}>
              ...
              <div className="task-fields">
                <input type="text" className="task-title-input" value={task.title}
                  ...
                  placeholder="할 일을 입력하세요" autoFocus={i === tasks.length - 1} />
                <div className="task-meta-row">
                  <input type="date" className="task-meta-input" value={task.date} onChange={e => updateTask(task.id, "date", e.target.value)} />
                  <input type="text" inputMode="numeric" className="task-meta-input" value={task.estimatedMin || ""}
                    onChange={e => { const v = e.target.value.replace(/[^0-9]/g, ""); updateTask(task.id, "estimatedMin", v === "" ? 0 : Math.min(480, parseInt(v, 10))); }}
                    placeholder="예상 시간" style={{ width: 80 }} />
                  <span style={{ fontSize: 12, color: T.textMuted, alignSelf: "center" }}>분</span>
                </div>
              </div>
              ...
              return (
                <li key={b.id} className={`timeline-item ${dragIdx === i ? "timeline-item--dragging" : ""} ${dragOverIdx === i ? "timeline-item--dragover" : ""}`}
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    ...
                    <span style={{ flex: 1, fontSize: 13, fontWeight: 500, color: T.textPrimary }}>{b.title}</span>
