...
import { T, PASTELS, CATEGORIES, uid, formatMin, formatDate, getSpeedY, getSpeedStatus, getSpeedStatusFromMins } from './constants';
...
function RecordPage({ records = [], savedIds = [], onToggleSave, onNavigate, onRemoveRecord }) {
  ...
  const filtered = useMemo(() => {
    let list = filter === 'all' ? safeRecords : safeRecords.filter(r => (r?.categories || []).includes(filter));
    list = [...list];
...
function AnalysisPage({ records = [] }) {
  ...
  const catSpeeds = useMemo(() => {
    ...
    for (const r of records) {
      ...
      for (const b of r.breakdown) {
        ...
        if (!m[cat]) m[cat] = { totalEst: 0, totalAct: 0 };
  ...
  const pieData = useMemo(() => {
    ...
    for (const r of records) {
      ...
      for (const b of r.breakdown) {
        ...
        m[key].min += Number.isFinite(b.actMin) ? b.actMin : 0;
      }
    }
  ...
  const totalSummary = useMemo(() => {
    if (!Array.isArray(records)) return { totalEst: 0, totalAct: 0, status: null };
    const totalEst = records.reduce((s, r) => s + (Number.isFinite(r?.totalEstMin) ? r.totalEstMin : 0), 0);
    const totalAct = records.reduce((s, r) => s + (Number.isFinite(r?.totalActualMin) ? r.totalActualMin : 0), 0);
...
function SettingsSection({ title, subtitle, children }) {
  return (
    <section style={{ marginBottom: 36 }}>
      ...
      {subtitle && (
        <p style={{ margin: 0, marginBottom: 18, fontSize: 12.5, color: T.color.textMuted, lineHeight: 1.6 }}>
          {subtitle}
        </p>
      )}
...
function SettingsRow({ title, description, children, statusBadge }) {
  return (
    <div style={{
      ...
      {description && (
        <p style={{ margin: 0, marginBottom: 14, fontSize: 12, color: T.color.textMuted, lineHeight: 1.6 }}>
          {description}
        </p>
      )}
      {children}
...
function Badge({ color, children }) {
  const palette = {
    primary: { bg: 'rgba(0, 82, 45, 0.08)', fg: T.color.primary, bd: 'rgba(0, 82, 45, 0.16)' },
    warn: { bg: 'rgba(196, 73, 73, 0.08)', fg: '#C44949', bd: 'rgba(196, 73, 73, 0.20)' },
    muted: { bg: 'rgba(24, 29, 25, 0.04)', fg: T.color.textMuted, bd: 'rgba(24, 29, 25, 0.08)' },
  };
