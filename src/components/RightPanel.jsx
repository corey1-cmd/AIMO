...
import { uid, T, formatMin, CATEGORIES } from '../constants';
  ...
  const handleAnalyze = () => {
    ...
    setTimeout(() => {
      ...
      try {
        ...
        const r = runAnalysis(mapped, learnData);
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          boxShadow: `0 0 16px ${T.color.electricMint}`,
        }} />
      </div>
          ...
          return (
            <div key={s} style={{
              ...
              <span aria-hidden style={{
                width: 10, height: 10,
                borderRadius: '50%',
                background: done ? T.color.primary : (active ? T.color.electricMint : 'rgba(24, 29, 25, 0.18)'),
              ...
              <span style={{
                fontSize: 12.5,
                color: done ? T.color.textPrimary : (active ? T.color.textPrimary : T.color.textMuted),
                fontWeight: active ? T.font_.weight.medium : T.font_.weight.regular,
              }}>{l}</span>
              {done && <span style={{ marginLeft: 'auto', color: T.color.primary, fontSize: 12 }}>✓</span>}
            </div>
          );
        })}
      </div>
            ...
            return (
              <li key={b.id || i} style={{
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 13, fontWeight: T.font_.weight.medium, color: T.color.textPrimary, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    {b.v3?.isFixed && <span style={{ marginRight: 4 }}>🔒</span>}
                    {b.title}
                  </div>
                  <div style={{ fontSize: 10.5, color: T.color.textMuted, marginTop: 2 }}>
...
function SummaryStat({ label, value, suffix, mono, accent }) {
  return (
    <div style={{
      ...
      <span style={{ fontSize: 11, color: T.color.textMuted, letterSpacing: '0.02em' }}>{label}</span>
      <span style={{
        fontSize: 22,
        fontWeight: T.font_.weight.semibold,
        ...
        {value}{suffix && <span style={{ fontSize: 12, fontWeight: T.font_.weight.regular, color: T.color.textMuted, marginLeft: 3 }}>{suffix}</span>}
      </span>
    </div>
  );
}
