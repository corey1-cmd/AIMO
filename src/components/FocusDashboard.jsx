...
import { T2, T, CATEGORIES, formatMin, getSpeedY, getSpeedStatus, getSpeedStatusFromMins } from '../constants';
...
function SessionTimelineCard({ plan, elapsedOffsetSec = 0, paused }) {
  ...
  function actualColor(d, status) {
    if (!d.isCurrent) return T2.color.primary;
    if (!status) return T2.color.accent;
    if (status.type === 'fast') return T2.color.primary;
    if (status.type === 'slow') return '#C97A4A';
    return T2.color.accent;
  }
  ...
  return (
    <CardLight>
      ...
      {/* 범례 */}
      <div style={{ display: 'flex', gap: 14, marginBottom: 4, fontSize: 11, color: T2.color.textMuted, flexWrap: 'wrap' }}>
        <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}>
          <span style={{ width: 12, height: 8, background: 'rgba(24,29,25,0.12)', display: 'inline-block', borderRadius: 2 }} />
          예상
        </span>
        <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}>
          <span style={{ width: 12, height: 8, background: T2.color.primary, display: 'inline-block', borderRadius: 2 }} />
          실제
        </span>
        <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}>
        ...
        >
          ...
          {[0, maxMin / 2, maxMin].map((v, i) => (
            <g key={i}>
              ...
              <text x={padL - 6} y={y(v) + 4} textAnchor="end" fill={T2.color.textMuted} fontSize="10" fontFamily={T2.font.familyMono}>{fmtHm(v)}</text>
            </g>
          ))}
          ...
          {data.map((d, i) => (
            <text
              ...
              y={H - 8}
              textAnchor="middle"
              fill={d.isCurrent ? T2.color.primary : T2.color.textMuted}
              fontSize="10"
              fontFamily={T2.font.familyMono}
          ...
          {tip && (
            <g>
              ...
              />
              <text x={tip.x + 12} y={tip.y + 18} fill={T2.color.text} fontSize="11" fontWeight="600">
                {tip.d.idx + 1}. {(tip.d.title || '').slice(0, 16)}{(tip.d.title || '').length > 16 ? '…' : ''}
              </text>
              <text x={tip.x + 12} y={tip.y + 36} fill={T2.color.textMuted} fontSize="10">예상</text>
              <text x={tip.x + tip.w - 12} y={tip.y + 36} textAnchor="end" fill={T2.color.text} fontSize="10" fontFamily={T2.font.familyMono}>{fmtHm(tip.d.estMin)}</text>
              <text x={tip.x + 12} y={tip.y + 52} fill={T2.color.textMuted} fontSize="10">실제</text>
              <text x={tip.x + tip.w - 12} y={tip.y + 52} textAnchor="end" fill={T2.color.text} fontSize="10" fontFamily={T2.font.familyMono}>
                {tip.d.actMin > 0 ? fmtHm(tip.d.actMin) : '—'}
              </text>
              <text x={tip.x + 12} y={tip.y + 70} fill={T2.color.primary} fontSize="10">속도</text>
              <text
                x={tip.x + tip.w - 12} y={tip.y + 70} textAnchor="end"
                fill={tip.status?.type === 'slow' ? '#C97A4A' : T2.color.primary}
                fontSize="10" fontFamily={T2.font.familyMono} fontWeight="600"
              >
                {tip.status ? `${tip.status.y.toFixed(2)}%` : '—'}
              </text>
              {tip.d.isFixed && (
                <text x={tip.x + 12} y={tip.y + 86} fill="#D43F3F" fontSize="9" fontFamily={T2.font.familyMono} fontWeight="600">
                  🔒 고정 시간
                </text>
              )}
            </g>
...
function LiveTimerCard({ plan, elapsedOffsetSec = 0, paused, onTogglePause }) {
  ...
  return (
    <CardLight style={{
      border: `1px solid ${T2.color.primary}`,
      ...
      <CardTitle right={
        <span style={{
          ...
          fontFamily: T2.font.familyMono,
          letterSpacing: '0.06em',
          color: paused ? '#C97A4A' : T2.color.primary,
          background: paused ? 'rgba(201, 122, 74, 0.10)' : T2.color.surfaceSoft,
          padding: '2px 8px',
          borderRadius: 9999,
      ...
      <div style={{ fontSize: 10.5, color: T2.color.textMuted, fontFamily: T2.font.familyMono, letterSpacing: '0.04em', marginBottom: 4 }}>
      ...
      <div style={{
        ...
        <div style={{
          ...
          fontSize: 44,
          fontWeight: 700,
          color: overTime ? '#C97A4A' : T2.color.primary,
          lineHeight: 1,
        ...
        <div style={{ fontSize: 10, color: T2.color.textMuted, marginTop: 6 }}>
          {paused ? '일시 정지됨' : '경과 시간'}
        </div>
      </div>
      ...
      <button
        ...
        style={{
          width: '100%',
          padding: '9px 14px',
          background: paused ? T2.color.primary : 'rgba(255, 255, 255, 0.65)',
          color: paused ? 'white' : T2.color.textSecondary,
          border: paused ? 'none' : `1px solid ${'rgba(47, 36, 30, 0.14)'}`,
          borderRadius: 10,
          cursor: 'pointer',
          fontSize: 12.5,
          fontWeight: T2.font.weightSemibold,
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 6 }}>
        <span style={{ fontSize: 11, color: T2.color.textMuted }}>
          예상 {formatMin(estMin)}
        ...
        <span style={{
          ...
          fontFamily: T2.font.familyMono,
          fontWeight: T2.font.weightSemibold,
          color: !status ? T2.color.textMuted
            : status.type === 'fast' ? T2.color.primary
      ...
      {status && (
        <div style={{
          ...
          borderRadius: 10,
          fontSize: 11.5,
          color: status.type === 'fast' ? T2.color.primary
  ...
  return (
    <CardLight>
      <CardTitle right={fixedSteps.length > 0 && (
        <span style={{ fontSize: 10, color: T2.color.textMuted, fontFamily: T2.font.familyMono }}>
              ...
              <div key={it.id} style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
                  <span aria-hidden style={{
                    ...
                    color: isPassed ? T2.color.primary : '#D43F3F',
                    display: 'inline-flex',
                    alignItems: 'center',
                    justifyContent: 'center',
                    fontSize: 11,
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{
                      fontSize: 12.5,
                      fontWeight: T2.font.weightSemibold,
                      color: isPassed ? T2.color.textMuted : T2.color.text,
                      overflow: 'hidden',
                      textOverflow: 'ellipsis',
                      whiteSpace: 'nowrap',
                    }}>{stripPrefix(it.title)}</div>
                    <div style={{ fontSize: 10.5, color: T2.color.textMuted, fontFamily: T2.font.familyMono }}>
                  ...
                  <span style={{
                    ...
                    fontFamily: T2.font.familyMono,
                    fontWeight: T2.font.weightSemibold,
                    color: isPassed ? T2.color.textMuted
                      : isCurrent ? '#D43F3F'
                      : isUrgent ? '#C97A4A'
                      : T2.color.primary,
                  }}>
    ...
    <CardLight>
      ...
      {status && (
        <div style={{
          ...
          borderRadius: 10,
          fontSize: 11.5,
          color: status.type === 'fast' ? T2.color.primary : '#A8602F',
          textAlign: 'center',
          fontWeight: T2.font.weightMedium,
        }}>{status.message}</div>
      )}
...
function Mini({ label, value, accent, warn }) {
  return (
    <div style={{
      ...
      <div style={{ fontSize: 10.5, color: T2.color.textMuted, marginBottom: 4 }}>{label}</div>
      <div style={{
        fontSize: 18,
        fontWeight: T2.font.weightSemibold,
        color: accent ? T2.color.primary : warn ? '#A8602F' : T2.color.text,
        fontFamily: T2.font.familyMono,
  ...
  return (
    <CardLight>
            ...
            return (
              <div key={it.id || i} style={{
                ...
                <span style={{
                  flex: 1, minWidth: 0,
                  fontSize: 12.5,
                  color: i === 0 ? T2.color.text : T2.color.textSecondary,
                  fontWeight: i === 0 ? T2.font.weightMedium : T2.font.weightRegular,
                  overflow: 'hidden',
                  textOverflow: 'ellipsis',
                  whiteSpace: 'nowrap',
      ...
      <button
        ...
        onMouseLeave={(e) => { e.currentTarget.style.background = T2.color.primary; e.currentTarget.style.transform = 'translateY(0)'; }}
      >
...
function SessionProgressCard({ plan }) {
  ...
  return (
    <CardLight>
      ...
      <div style={{ fontSize: 11, color: T2.color.textMuted, fontFamily: T2.font.familyMono }}>
