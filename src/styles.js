...
export function buildCSS(T) { return `
  ...
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
  ...
  .shell{min-height:100vh;padding-top:64px}
  .page{max-width:960px;margin:0 auto;padding:32px 24px 64px}
  @media(max-width:768px){.shell{padding-top:72px}.page{padding:28px 16px 64px}.nav-links{gap:0}.nav-link{padding:6px 10px;font-size:12px}}

  /* ── Main Hero + Cards (틸트 적용) ── Sprout h1: 40px, -0.02em, 700 */
  .main-hero{text-align:center;padding:48px 0 36px}
  ...
  .main-cards{display:grid;grid-template-columns:repeat(3,1fr);gap:16px;max-width:720px;margin:0 auto;padding-top:8px}
  .main-card{display:flex;flex-direction:column;align-items:center;justify-content:flex-start;gap:14px;padding:32px 20px;background:${T.bgSurface};border:none;border-radius:${T.radiusCard};cursor:pointer;transition:transform 0.2s,box-shadow 0.2s;min-height:200px;overflow:hidden;color:inherit;position:relative;box-shadow:${T.shadowAmbient};text-align:center}
  .main-card:hover{transform:translateY(-2px);box-shadow:${T.shadowRaised}}
  .main-card-icon{width:64px;height:64px;border-radius:18px;display:flex;align-items:center;justify-content:center;font-size:32px;line-height:1;position:relative;z-index:1;flex-shrink:0}
  ...
  .main-card-title{font-size:15px;font-weight:700;letter-spacing:-0.005em;position:relative;z-index:1;line-height:1.3;min-height:2.6em;display:flex;align-items:center;justify-content:center;text-align:center}
  .main-card-desc{font-size:12px;color:${T.textSecondary};text-align:center;line-height:1.5;position:relative;z-index:1;margin-top:auto}
  .main-card-body{display:flex;flex-direction:column;align-items:center;gap:8px;flex:1;width:100%}
  /* 모바일: 3개 카드를 1열로 — 가로 배치하면 한 장만 외롭게 떠있는 문제 해결 */
  @media(max-width:640px){
    .main-cards{grid-template-columns:1fr;gap:12px;padding-top:0}
    .main-card{flex-direction:row;justify-content:flex-start;align-items:center;text-align:left;gap:18px;min-height:auto;padding:20px 22px}
    .main-card-icon{width:56px;height:56px;font-size:28px;border-radius:16px}
    .main-card-title{min-height:auto;text-align:left;justify-content:flex-start;font-size:16px}
    .main-card-desc{margin-top:4px;text-align:left}
    .main-card-body{flex:1;display:flex;flex-direction:column;gap:2px;min-width:0}
    .main-hero{padding:32px 0 24px}.main-title{font-size:34px}
  }
  ...
  .top3-item{display:flex;align-items:center;gap:14px;padding:14px 18px;background:${T.bgSurface};border:none;border-radius:18px;cursor:pointer;position:relative;transition:transform 0.2s,box-shadow 0.2s;box-shadow:${T.shadowAmbient};min-height:72px}
  ...
  .top3-body{flex:1;display:flex;flex-direction:column;gap:4px;min-width:0;position:relative;z-index:1;justify-content:center}
  .top3-t-title{font-size:14px;font-weight:600;color:${T.textPrimary};overflow:hidden;text-overflow:ellipsis;white-space:nowrap;line-height:1.3}
  .top3-t-meta{font-size:11px;color:${T.textMuted};font-family:${T.mono};line-height:1.4;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
  ...
  /* ── Distill 모바일: 5+ 항목 세로 스크롤 폭발 방지 — 가로 스냅 캐러셀 ── */
  @media(max-width:768px){
    ...
    .timeline-item{
      ...
      flex-shrink:0;width:78%;max-width:300px;min-height:188px;
      padding:16px !important;border-radius:18px !important;
      background:${T.bgSurface};border:none !important;
      box-shadow:${T.shadowAmbient};
      scroll-snap-align:start;
      cursor:default !important;
    }
    .timeline-item:hover{background:${T.bgSurface};box-shadow:${T.shadowRaised}}
    /* 카드 내부 제목줄 wrap 허용 + 드래그 핸들은 모바일에서 숨김 */
    .timeline-item .timeline-handle{display:none}
    .timeline-item-title{
      font-size:14px !important;font-weight:600;line-height:1.35;
    ...
    .timeline-list-hint{
      ...
      padding:0 0 8px 4px;letter-spacing:0.02em;
    }
    /* 모바일 카드에서는 chip row, bar 의 데스크톱 들여쓰기(paddingLeft:58) 무효화 */
    .timeline-item > div[style*="paddingLeft"]{padding-left:0 !important}
  }
  /* 데스크톱에서는 hint 숨김 */
  .timeline-list-hint{display:none}
