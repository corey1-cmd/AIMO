...
export function buildCSS(_T) {
  // Phase 2.D: remap v1 T fields → v2 T2 (Sepia Neutral) at runtime.
  // This preserves all existing CSS class definitions while swapping the
  // underlying color values across the entire stylesheet.
  // After Phase 2.E this entire buildCSS will be rewritten using T2 directly.
  const T = {
    // Brand
    accent:        '#2F241E',  // primary (sepia brown) — replaces deep forest
    accentHover:   '#1F1612',  // darker sepia
    accent2:       '#8C6F5A',  // muted warm brown — replaces electric mint
    accent3:       '#1F1612',  // dark sepia
    accentSoft:    '#EAE5DF',  // surfaceSoft
    accentGlow:    'rgba(140, 111, 90, 0.18)',  // accent glow
    ...
    border:        '#E2DDD7',
    borderLight:   '#EAE5DF',
    currentBorder: '#8C6F5A',

    // Status
    success:       '#6E8B74',  // desaturated olive
    error:         '#8A4A4A',  // muted red-brown
    errorSoft:     'rgba(138, 74, 74, 0.10)',
    warningSoft:   'rgba(166, 124, 82, 0.12)',
    ...
    // Shadow
    shadow:        '0 2px 8px rgba(28, 26, 24, 0.06)',
  };
  return `
  ...
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
  ...
  body{
    ...
    min-height:100vh;
    /* Atelier Cyan: 차가운 오프화이트 + ambient orb 네 개 (그리드 없음, 빛만) */
    background-color:${T.bgRoot};
    background-image:
      radial-gradient(at 8% 6%, rgba(140, 111, 90, 0.10) 0px, transparent 38%),
      radial-gradient(at 92% 94%, rgba(47, 36, 30, 0.08) 0px, transparent 44%),
      radial-gradient(at 78% 8%, rgba(234, 229, 223, 0.45) 0px, transparent 38%),
      radial-gradient(at 18% 88%, rgba(140, 111, 90, 0.06) 0px, transparent 36%),
      linear-gradient(${T.bgRoot}, ${T.bgRoot});
    background-attachment:fixed;
    position:relative;
  }
  /* 그리드 격자 제거 — 빛(orb)과 그림자만으로 깊이감 표현 */
  body > *{position:relative;z-index:1}
  /* 스크롤바 (Webkit) — 가는, 미래지향적 */
  ::-webkit-scrollbar{width:10px;height:10px}
  ::-webkit-scrollbar-track{background:transparent}
  ::-webkit-scrollbar-thumb{background:rgba(47, 36, 30, 0.18);border-radius:9999px;border:2px solid transparent;background-clip:padding-box}
  ::-webkit-scrollbar-thumb:hover{background:rgba(47, 36, 30, 0.32);background-clip:padding-box;border:2px solid transparent}
  /* 텍스트 선택 색 */
  ::selection{background:rgba(140, 111, 90, 0.22);color:${T.textPrimary}}
