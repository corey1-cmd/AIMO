...
import { isTravelStep as __isTravelStep, extractPlaceFromTitle as __extractPlace } from './geo.js';
...
export function runAnalysis(tasks, learningData) {
  ...
  // ── Phase Z: 이동 단계 감지 + 시간 보정 (geo.js 연동, GPS 데이터 옵션) ──
  // 단계 제목이 이동 행동이면 isTravel 플래그를 달고, 카테고리도 'move' 로 강제 설정.
  // 사용자 GPS 위치 + 장소 좌표 사전이 있으면 거리 기반으로 시간 재계산 (향후 확장).
  for (const item of breakdown) {
    if (!item || item.isMarker) continue;
    if (typeof __isTravelStep === 'function' && __isTravelStep(item.title)) {
      item.isTravel = true;
      // 카테고리 강제 세팅 (이미 move 면 그대로)
      if (item.category !== 'move') {
        item.category = 'move';
      }
      // 추출된 장소 키워드 저장 (UI 노출용)
      if (typeof __extractPlace === 'function') {
        const place = __extractPlace(item.title);
        if (place) item.placeKeyword = place;
      }
    }
  }
