...
export function runAnalysis(tasks, learningData) {
  ...
  // 2) 고정 시간 태스크 → v3 fixedEvents 로 변환
  //    먼저 파싱만 선행해서 title → L4 태그를 얻고, 태그 기준으로 anchor 생성
  let preParse;
  try {
    preParse = analyze(text, { now: nowHHMM(), optimize: false, strictTimeEnforcement: false });
  } catch (e) {
    console.warn("[engine] pre-parse threw, falling back:", e?.message || e);
    return runAnalysisLegacy(tasks, learningData);
  }

  const titleToTag = new Map();
  (preParse?.parsedDetails || []).forEach(d => {
    if (d.matched && d.l4) titleToTag.set(d.input, d.l4);
  });

  const fixedEvents = [];
  for (const t of valid) {
    if (t.hasFixed && t.fixedTime) {
      const tag = titleToTag.get(t.title.trim());
      if (tag) {
        fixedEvents.push({ tag, startAt: t.fixedTime });
      } else {
        console.info(`[engine] fixedTime specified for "${t.title}" but v3 couldn't tag it; ignoring anchor`);
      }
    }
  }

  // 3) 본 분석 (fixedEvents 포함)
  let v3;
  try {
    v3 = analyze(text, {
      now: nowHHMM(),
  ...
  // 6) timeline → breakdown (마커 없이 실제 스텝만 노출)
  const breakdown = realSteps.map((step, i) => {
    const parent = tagToInput.get(step.tag) || tagToLabel(step.l3 || step.tag);
    return v3StepToAction(step, i, parent);
  });
