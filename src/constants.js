...
/* ═══════════════════════════════════════════════════════════════
   ...
   x  : 입력 속도 퍼센트 = (실제 시간 / 예상 시간) × 100
   y  : 효율값 = round(10000/x, 2)   [소수점 둘째 자리까지]
   d  : 100과 y의 절대 차이 (둘째 자리까지)
...
/** x → y 변환 (역수 모델, 둘째 자리 반올림). x≤0이면 null. */
export function getSpeedY(x) {
  if (x == null || !isFinite(x) || x <= 0) return null;
  return Math.round((10000 / x) * 100) / 100;
}

/**
 * x를 받아 효율 / 차이 / 메시지 / 분류 반환.
 * @returns { y, diff, message, type } | null
 *   type: 'fast' | 'slow' | 'normal'
 *   diff: number (parseFloat 처리, .00 제거됨)
 */
export function getSpeedStatus(x) {
  const y = getSpeedY(x);
  if (y == null) return null;
  // 사용자 명세 그대로: Math.abs(100 - y).toFixed(2) → parseFloat
  const diff = parseFloat(Math.abs(100 - y).toFixed(2));
  if (y < 100) {
    return { y, diff, type: 'slow', message: `${diff}% 느리게 진행되고 있어요!` };
  }
  if (y > 100) {
    return { y, diff, type: 'fast', message: `${diff}% 빠르게 진행되고 있어요!` };
  }
  return { y, diff: 0, type: 'normal', message: '정상 속도로 진행 중입니다.' };
}

/**
 * 예상시간(estMin) 대비 실제(actualMin) 비율을 받아 status 반환.
 * 실제 0이면 null.
 */
export function getSpeedStatusFromMins(estMin, actualMin) {
  if (!actualMin || actualMin <= 0 || !estMin || estMin <= 0) return null;
  const x = (actualMin / estMin) * 100;
  return getSpeedStatus(x);
}


/* ═══════════════════════════════════════════════════════════════
   T2 — v2 design tokens (Phase 2 design refresh, Sepia Neutral)
   v1 (T) is preserved for backwards compatibility. v2 consumers
   should import T2 directly. v1 will be removed in Phase 2.E.
   ═══════════════════════════════════════════════════════════════ */
export const T2 = {
  color: {
    // Brand
    primary:        '#2F241E',  // deep sepia brown
    accent:         '#8C6F5A',  // muted warm brown

    // Surfaces
    surface:        '#F4F1EC',  // warm paper, page background
    surfaceRaised:  '#FFFFFF',  // cards, tables
    surfaceSoft:    '#EAE5DF',  // section separation without borders
    ...
    // Status
    statusGood:     '#6E8B74',  // desaturated olive
    statusWarn:     '#A67C52',  // soft amber brown
    statusCrit:     '#8A4A4A',  // muted red-brown
  },

  font: {
    familyDisplay: '"Plus Jakarta Sans", "Noto Sans KR", system-ui, sans-serif',
    familyBody:    '"Plus Jakarta Sans", "Noto Sans KR", system-ui, sans-serif',
    familyMono:    '"JetBrains Mono", "Fira Code", ui-monospace, monospace',

    sizeDisplay:   32,   // page H1
    sizeHeading:   18,   // section H2
    sizeBody:      14,   // default
    sizeCaption:   12,   // meta, labels
    sizeMono:      13,   // numbers, durations

    weightRegular:  400,
    weightMedium:   500,
    weightSemibold: 600,

    lineHeight: { display: 1.1, heading: 1.3, body: 1.5, caption: 1.4 },
    tracking:   { tight: '-0.02em', tightest: '-0.04em' },
  },

  // 4px base grid; only these values are allowed
  space: { 1: 4, 2: 8, 3: 12, 4: 16, 5: 20, 6: 24, 8: 32, 10: 40, 14: 56, 20: 80 },

  radius: { sm: 6, md: 10, lg: 16, pill: 9999 },

  shadow: {
    sm: '0 1px 2px rgba(28, 26, 24, 0.04)',
    md: '0 2px 8px rgba(28, 26, 24, 0.06)',
    lg: '0 8px 24px rgba(28, 26, 24, 0.08)',
  },

  // Decoration policy: 'none' on data surfaces, 'subtle' on landing/empty/settings
  decoration: { dataSurface: 'none', landing: 'subtle' },
};
